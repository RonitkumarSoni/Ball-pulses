...
const shadowMesh = new THREE.Mesh(new THREE.PlaneGeometry(2.5, 2.5), shadowMaterial);
...
loader.load('/models/basketball.glb', (gltf) => {
  ...
  ball.traverse((child) => {
    if (child.isMesh && child.material) {
      ...
      if (m.color) m.color.set(0xd65a31); // explicitly set to basketball orange
      m.needsUpdate = true;
    }
  });
...
function hideLoader() {
  ...
  if (loaderEl && loaderEl.style.visibility !== 'hidden') {
    ...
    setTimeout(() => loaderEl.style.visibility = 'hidden', 600);
  }
}
...
statCards.forEach((card, i) => {
  gsap.set(card, {
    y: statStackOffsets[i].y + '%',
    rotation: statStackOffsets[i].rotate,
    scale: statStackOffsets[i].scale,
    opacity: i === 0 ? 1 : 0.6,
    zIndex: statCards.length - i,
  });
...
gsap.to('.how-bg-text', {
  y: -100, ease: 'none',
  scrollTrigger: { trigger: '#how-section', start: 'top bottom', end: 'bottom top', scrub: true }
});

// How — step-item cards stacking from TOP (same premium effect)
...
stepCards.forEach((card, i) => {
  gsap.set(card, {
    y: stepStackOffsets[i].y + '%',
    rotation: stepStackOffsets[i].rotate,
    scale: stepStackOffsets[i].scale,
    opacity: i === 0 ? 1 : 0.5,
    zIndex: stepCards.length - i,
  });
...
function animate() {
  ...
  if (ball) {
    ...
    shadowMesh.position.y = ball.position.y - (1.1 * ball.scale.x / baseScale);
    shadowMesh.scale.setScalar(ball.scale.x / baseScale);
    shadowMaterial.opacity = (ball.scale.x / baseScale) * 0.6;
  }
